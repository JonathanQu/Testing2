...
globals [DisplayHealth DisplayFoodLeft] ; defines the new global variables DisplayHEalth and DisplayFoodLeft
...
to changeDisplayHealth
  set DisplayHealth 1 ; change the display health to 1
end

to changeDisplayFoodLeft
  set DisplayFoodLeft 1 ; change the displayfoodleft to 1
end
...
to run-reds ;turtle context
  ...
  wiggle
  LayBombRed
  if not (DisplayHealth = 1) ; if the display health is not equal to 1
...
to run-aid
  ...
  [if random 100 > aid_rate ; then check if random 100 is greater than the aid_rate
    [set foodLeft (foodLeft + 50) ; if thats the case then set the food left to foodleft +50. increases it
      ifelse DisplayFoodLeft = 1 ;
      [set pcolor brown] ; if food left is equal to 1 then set it to brown
      [set pcolor scale-color brown foodLeft 0 100]]] ; otherwise set the pcolor to a scale-color of brown.
end
...
to death ; each time it goes on a bomb it loses 10 life points
 set life (life - (random 10)) ;if death is called, make a turtle lose between 0-9 health
  ...
  [if shade-of? brown pcolor ; if  the patch is a shade of brown.
    [ifelse foodLeft - x < 0 ; if foodleft - x is less than 0
      [set life (life + (foodLeft - x)) ; increase the life by foodleft-x
        set foodLeft 0] ; and then set the foodleft to 0

     [ set life (life + x) ; set the life to increase by x
      set foodLeft (foodLeft - x)] ; set the foodleft to lose x. opposite of above.

      ifelse DisplayFoodLeft = 1 ; if the displayfoodleft is 1
      [set pcolor brown] ; change the pcolor to brown
      [set pcolor scale-color brown foodLeft 0 100]]] ; otherwise do this
  if pcolor = 30 [set pcolor black] ;if pcolor is 30 set the pcolor to black.
end
...
Ms. Genkina
Thanks to jonathan for working on many of the functions. thanks to bayan for working on the breeds and their properties and descriptive comments and the info tab, thanks to leo for working on making it look like the skeleton.

##Improvements in Version 2

-In version 2 we included the bomb defusers for each team.
-each brown patch begins with a specific amount of food, which is controlled by a slider.
-when a turtle lands on a brown patch, the food supply is decreased by the amount that the turtle is increased
- a turtle loses a random amount of life points when it lands on a bomb
- you can switch the displaying of food on or off.
- the user can switch the displaying of turtle life on or off.
- we incorporated turtle shapes.
@#$#@#$#@
default
true
0
