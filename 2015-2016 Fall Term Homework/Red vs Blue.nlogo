; Team Squirrels: Jonathan Quang, Yuanchu Liu, Bayhan Berri
; IntroCS1 pd7
...
to setup
  ca ;clears the world
  crt num_reds [ setup-red] ;creates num_reds amount of turtles and has them do their setup
  crt num_blues [ setup-blue] ;does the same as above line, but for the blue turtles
end
...
to wiggle ;turn randomly, move forward randomly, and wait a random amount
  rt random 20
  lt random 20
  fd random 4
  wait .1
...
to checkBombs [whatColor] ;function checks if a turtle is on a patch with the given color, if it is,
  if pcolor = whatColor          ;the patch is colored black and the turtle dies
[ set pcolor black
  die]
end
...
to red_team_functions ;has turtles wriggle, check for the opposite teams bomb color, and lay red bombs
wriggle
checkBombs  blue
LayBombRed
end

to blue_team_functions ;has turtles wriggle, check for the opposite teams bomb color, and lay blue bombs
wriggle
checkBombs red
LayBombBlue
end
...
to DecideWinner ;if all turtles of any of the two colors are dead, label the remaining as winners and make them draw
  if (TurtlesAlive blue = 0) or (TurtlesAlive red = 0)
  [set label "WINNER"
    pd ]
end
...
For fairness, all sliders should be adjusted to the desired setting before hitting the setup button. After hitting the setup button, you may hit the go button to turn on the simulation.


## CREDITS AND REFERENCES

Credit to Ms.Genkina for teaching computer science and the netlogo turtle models library for providing the default turtle shape.
@#$#@#$#@
default
true
0
